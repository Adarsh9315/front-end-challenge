...
const App = () => {
	...
	const [activePage, setActivePage] = useState('nominations');
	...
	return (
		<div className='container-fluid movie-app'>
			<div className='row d-flex align-items-center mt-4 mb-3'>
				<MovieListHeading heading='Movie Night' />
				<div className='col d-flex justify-content-end page-tabs'>
					<button
						className={`page-tab ${activePage === 'nominations' ? 'active' : ''}`}
						onClick={() => setActivePage('nominations')}
						type='button'
					>
						Nominations
					</button>
					<button
						className={`page-tab ${activePage === 'showdown' ? 'active' : ''}`}
						onClick={() => setActivePage('showdown')}
						type='button'
					>
						Showdown
					</button>

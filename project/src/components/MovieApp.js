...
const MovieApp = () => {
	...
	const addNominationMovie = (movie) => {
		...
		if (savedNominations) {
			...
			if (savedNominations.length === 5) {
				alert('Only 5 nominations are allowed per user');
				return;
			}
	...
	const nominationsLength = JSON.parse(localStorage.getItem('nominations') || '[]').length;
	...
	return (
		<div className='container-fluid movie-app'>
			...
			<div className='banner' style={{display: nominationsLength === 5 ? 'block' : 'none'}}>
